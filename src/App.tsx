...
export default function App() {
  const [cameraGroups, setCameraGroups] = useState<CameraGroup[]>([
    { id: Date.now().toString(), count: 32, resolution: "8", bitrate: 8, fps: 30, compression: "H.265+ High Efficiency" }
  ]);
  ...
  const results = useMemo(() => {
    ...
    cameraGroups.forEach(group => {
      // Use FPS multiplier relative to 30 FPS default
      const fpsMultiplier = (group.fps || 30) / 30;
      const gbPerCamDay = (group.bitrate * fpsMultiplier * 3600 * hoursPerDay) / (8 * 1024);
      dailyGB += (gbPerCamDay * group.count);
      totalTB += (gbPerCamDay * group.count * days) / 1024;
      
      weightedBitrateSum += (group.bitrate * fpsMultiplier * group.count);
      weightedFPSSum += ((group.fps || 30) * group.count);
    ...
    const avgFPS = totalBitrateCount > 0 ? Math.round(weightedFPSSum / totalBitrateCount) : 30;
  ...
  const calculateBitrate = (res: string, compression: string, customMP: number, fps: number = 30) => {
    const factor = compression === "H.264" ? 1.0 : compression === "H.265" ? 0.7 : 0.5;
    const fpsMultiplier = fps / 30;
    if (res === 'custom') {
      return (customMP * 2) * factor * fpsMultiplier; // Using MP * 2 as a consistent base
    }
    const resNum = parseFloat(res);
    const base = !isNaN(resNum) ? resNum * 2 : 4;
    return base * factor * fpsMultiplier;
  };
  ...
  const updateGroup = (id: string, updates: Partial<CameraGroup>) => {
    setCameraGroups(prev => prev.map(g => {
      ...
      // If resolution or compression or fps changed, recalculate bitrate if not manually overridden
      if (updates.resolution || updates.compression || updates.customMP || updates.fps !== undefined) {
         updated.bitrate = calculateBitrate(updated.resolution, updated.compression, updated.customMP || 12, updated.fps || 30);
      }
      return updated;
    }));
  };
  ...
  const generatePDF = () => {
    ...
    y += 10;
    drawGridItem("Avg. FPS Setting", `${results.avgFPS} FPS`, "mode", col1, y, colWidth);
    ...
    cameraGroups.forEach((group, idx) => {
      ...
      doc.setFont("helvetica", "normal");
      doc.text(`${group.compression} @ ${group.bitrate.toFixed(1)} Mbps (${group.fps} FPS)`, 70, y);
      
      const subtotal = ((group.bitrate * (group.fps / 30) * 3600 * (mode === '24' ? 24 : mode === 'motion' ? 12 : alarmHours) * group.count * days) / (8 * 1024 * 1024)).toFixed(2);
      doc.setTextColor(15, 23, 42);
      doc.setFont("helvetica", "bold");
      ...
      <div className="max-w-xl mx-auto pt-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input 
                          type="number" 
                          step="0.1"
                          value={group.bitrate.toFixed(1)}
                          onChange={(e) => updateGroup(group.id, { bitrate: Math.max(0.1, parseFloat(e.target.value) || 0) })}
                          className="w-full bg-zinc-900 border border-zinc-700/50 rounded-xl px-4 py-3 text-xl font-bold focus:border-emerald-500/50 outline-none text-white transition-all text-center"
                        />
                      </div>
                      <div>
                        <label className="block text-zinc-500 text-[10px] font-bold uppercase tracking-widest mb-2 ml-1">FPS Setting</label>
                        <div className="relative">
                          <input 
                            type="number" 
                            value={group.fps}
                            onChange={(e) => updateGroup(group.id, { fps: Math.max(1, parseInt(e.target.value) || 0) })}
                            className="w-full bg-zinc-900 border border-zinc-700/50 rounded-xl px-4 py-3 text-xl font-bold focus:border-emerald-500/50 outline-none text-white transition-all text-center"
                            placeholder="30"
                          />
                          <span className="absolute right-4 top-1/2 -translate-y-1/2 text-[8px] font-black text-zinc-600 uppercase">FPS</span>
                        </div>
                      </div>
                    </div>
                  </div>
        ...
        <div className="mt-8 bg-zinc-900 rounded-3xl p-8 border border-zinc-800 shadow-xl overflow-hidden relative">
          ...
          <div className="mt-8 space-y-3 max-w-lg mx-auto">
            <div className="flex justify-center gap-4 border-b border-zinc-800 pb-4 mb-4">
              <div className="text-center">
                <p className="text-[9px] font-black text-zinc-500 uppercase tracking-widest leading-none">Net Bitrate</p>
                <p className="text-sm font-black text-white mt-1">{results.avgBitrate} Mbps</p>
              </div>
              <div className="w-px h-8 bg-zinc-800" />
              <div className="text-center">
                <p className="text-[9px] font-black text-zinc-500 uppercase tracking-widest leading-none">Avg Frame Rate</p>
                <p className="text-sm font-black text-white mt-1">{results.avgFPS} FPS</p>
              </div>
            </div>
                  ...
                  <div className="bg-slate-50/80 rounded-xl p-6 md:p-10 border border-slate-100 mb-10">
                    ...
                    <div className="space-y-8">
                       ...
                       <div className="grid grid-cols-2 lg:grid-cols-4 gap-y-8 gap-x-6 pb-8 border-b border-slate-200">
                          ...
                          <div>
                            <p className="text-[9px] font-black text-slate-400 uppercase tracking-[0.1em] mb-1">Avg. FPS</p>
                            <p className="text-[13px] font-bold text-slate-900">{results.avgFPS} FPS</p>
                          </div>
                          <div>
                             ...
                             <div key={group.id} className="flex items-center justify-between bg-white p-4 rounded-xl border border-slate-200 shadow-sm">
                               <div className="flex items-center gap-4">
                                 ...
                                 <div className="flex-1">
                                   ...
                                   <p className="text-[9px] text-slate-500 font-bold uppercase tracking-tight">{group.compression} @ {group.bitrate.toFixed(1)} Mbps ({group.fps} FPS)</p>
                                 </div>
                               </div>
                               <div className="text-right">
                                  <p className="text-[9px] text-slate-400 font-bold uppercase tracking-widest">Subtotal</p>
                                  <p className="text-xs font-black text-slate-900">
                                    {((group.bitrate * (group.fps / 30) * 3600 * (mode === '24' ? 24 : mode === 'motion' ? 12 : alarmHours) * group.count * days) / (8 * 1024 * 1024)).toFixed(2)} TB
                                  </p>
                               </div>
                             </div>
                           ))}
